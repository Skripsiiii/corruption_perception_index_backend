...
@section('content')
    {{-- <div class="w-full">
        <div class="h-12 p-4 bg-black w-full">
            asd
        </div> --}}
    ...
    <script>
        ...
        var map = L.map('map', {
            ...
            maxZoom: 7,
            dragging: true
        }).setView([-2.548926, 118.0148634], 5.5);
        // Define the boundaries for dragging (replace with your desired boundaries)
        var southWest = L.latLng(-13.00, 94.00); // Bottom-left corner
        var northEast = L.latLng(8.00, 141.00); // Top-right corner
        var bounds = L.latLngBounds(southWest, northEast);
        map.setMaxBounds(bounds); // Set the maximum bounds for dragging

        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 19,
            attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
        }).addTo(map);
        var geoJsonData = @json($jsonData);
        ...
        var stateToIndexMap = {
            @foreach ($provincesCorruption as $p)
                "{{ $p->name }}": {{ $p->index_result }},
            @endforeach
        };

        // console.log("testing: " + stateToIndexMap["haha"]);

        function getColorByState(state) {
            var indexResult = stateToIndexMap[state];
            if (indexResult == undefined) indexResult = -1
        ...
        function getCorruptionLevel(indexResult, city) {
            ...
            if (indexResult >= 0 && indexResult <= 20) {
                ...
                tableDiv +=
                '<th>Status</th><th><div style="background-color:#5FB4BB" class="p-1 w-auto rounded-md font-semibold text-center text-white"> Tingkat Korupsi Sangat Rendah</div></th>';
            }
            else{
                tableDiv +=
        ...
        function zoomToFeature(e) {
            ...
            var layer = e.target;
            layer.bringToFront();
            var city = layer.feature.properties.state; // Replace "city_name" with the actual city name
            if (stateToIndexMap[city] != undefined) {
                var url = "/map/" + city;
        ...
        geojson = L.geoJson(geoJsonData, {
            style: function(e) {
                return {
                    fillColor: getColorByState(e.properties.state),
                    weight: 2,
                    opacity: 1,
                    color: 'white',
                    dashArray: '3',
        ...
        legend.onAdd = function(map) {
            ...
            for (var i = 0; i < grades.length - 1; i++) {
                div.innerHTML +=
                    ...
                    (i == 0 ? '' : grades[i] + '&ndash;' + grades[i + 1]) + ' ' + labels[i] + '<br>' + "</div>";
            }
